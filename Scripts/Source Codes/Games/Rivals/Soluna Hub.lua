...
FlyingSection:AddParagraph({
    ...
    if UISettings.ShowWarnings then
        ...
        elseif ShowWarning then
            ...
            Window:Dialog({
                Title = string.format("%s   PREMIUM  ", string.format(MonthlyLabels[os.date("*t").month], " Soluna")),
                Content = " Upgrade to unlock all Options  â Contact @ keepmesmerizing via Discord to buy",
                Buttons = {
                    {
                        Title = "Confirm"
                    }
...
function ESPLibrary:Initialize(_Character)
    ...
    if Head and Head:IsA("BasePart") and HumanoidRootPart and HumanoidRootPart:IsA("BasePart") and Humanoid then
        ...
        if IsInViewport then
            ...
            self.NameESP.Text = Aiming and IsReady(Target) and self.Character == Target and string.format("ð¯@%sð¯", self.Player.Name) or string.format("@%s", self.Player.Name)
